...
extension SkeletonLoadable where Self: ThemeProvider {
    func makeAnimationGroup(previousGroup: CAAnimationGroup? = nil) -> CAAnimationGroup {
        let animDuration: CFTimeInterval = 1.5
        let animation1 = CABasicAnimation(keyPath: #keyPath(CAGradientLayer.backgroundColor))
        animation1.fromValue = theme.colors.surfaceContainerLowest.cgColor
        animation1.toValue = theme.colors.surfaceContainerHighest.cgColor
        ...
        animation2.fromValue = theme.colors.surfaceContainerHighest.cgColor
        animation2.toValue = theme.colors.surfaceContainerLowest.cgColor
